...
const qrStudentScanSchema = object({
    body: object({
        qrCodeSessionId: string().required("QR Code Session ID is Required"),
        gateNo: number().required("Gate No is Required")
            .min(0, "Gate No must be at least 0 characters long")
            .max(1, "Gate No must be at most 1 characters long"),
    }),
    headers: object({
        "x-access-token": string().required("Token is Required"),
        "x-refresh-token": string().required("Refresh Token is Required"),
...
export {qrStudentCreateSchema,qrStudentScanSchema}
